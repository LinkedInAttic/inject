...
category: howto
permalink: /docs/0.5.1x/howto/cross_domain.html
---
...
Given the CDN location, we will need to use both [Inject.setModuleRoot](/docs/0.5.1/api/inject.setmoduleroot.html) and [Inject.setCrossDomain](/docs/0.5.1/api/inject.setcrossdomain.html) to configure this installation of Inject. `setModuleRoot` tells Inject your files are on another server, while `setCrossDomain` tells inject where the two helper files are that it will need for cross-domain downloading.
